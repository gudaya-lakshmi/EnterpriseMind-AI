...
PER_QUESTION_RESULTS_FILE = "deepeval_per_question.csv"
...
# Use 3 while testing. Change to None for all questions.
TEST_LIMIT = 3
...
6. If the retrieved context completely answers the question, provide the
   complete answer and stop.
7. Only state:
...
12. When answering financial questions, preserve the values and units exactly
    as they appear in the retrieved context. If you perform calculations,
    ensure the units remain consistent with the source data.
13. Use the fallback response only when the answer is completely absent
...
def create_metrics(judge_model: OllamaModel) -> list:
    """
    Create fresh DeepEval metric objects for each batch.

    Creating new objects prevents metric state from one batch from
    affecting another batch.
    """
...
def append_per_question_results(
    batch_results: list,
    batch_test_cases: list,
    question_numbers: list[int],
) -> None:
    """
    Print and save clean per-question metric scores.
    """

    rows = []

    for question_number, test_case, test_result in zip(
        question_numbers,
        batch_test_cases,
        batch_results,
    ):
        metric_map = {
            ...
            "Contextual Precision": None,
            "Contextual Recall": None,
        }

        for metric_data in getattr(test_result, "metrics_data", []) or []:
            metric_name = getattr(metric_data, "name", None)
            score = getattr(metric_data, "score", None)

            if metric_name in metric_map and score is not None:
                metric_map[metric_name] = float(score)

        row = {
            "Question Number": question_number,
            "Question": test_case.input,
            "Faithfulness": metric_map["Faithfulness"],
            "Answer Relevancy": metric_map["Answer Relevancy"],
            "Contextual Precision": metric_map["Contextual Precision"],
            "Contextual Recall": metric_map["Contextual Recall"],
            "Test Case Success": bool(
                getattr(test_result, "success", False)
            ),
        }
        ...
        print("\nPer-question scores")
        print(f"Question {question_number}: {test_case.input}")
        ...
        for metric_name in [
            ...
            print(f"  {metric_name}: {display_score}")
...
def save_final_summary(
    ...
    summary_lines.append(
        f"Per-question scores saved to: {PER_QUESTION_RESULTS_FILE}"
    )
...
def main() -> None:
    ...
    if TEST_LIMIT is not None:
        dataset = dataset.head(TEST_LIMIT).copy()
    ...
    for batch_start in range(
        ...
        try:
            ...
            append_per_question_results(
                batch_results=batch_results,
                batch_test_cases=batch_test_cases,
